    ...
    <q-page class="md:w-[80%] h-fit m-auto md:px-7 px-6">
        ...
        <div class="grid grid-cols-3 gap-3 bg-gray-800 rounded-t-lg">
            <p class="cursor-pointer text-center lg:text-xl md:text-lg text-md font-semibold py-3 lg:py-5 hover:bg-blue-400 rounded-t-lg"
                @click="$router.push('/wallet/deposit')">Deposit</p>
            <p class="cursor-pointer text-center lg:text-xl md:text-lg text-md font-semibold py-3 lg:py-5 hover:bg-blue-400 rounded-t-lg"
                @click="$router.push('/wallet/withdraw')">Withdraw</p>
            <p
                class="cursor-pointer text-center lg:text-xl md:text-lg text-md font-semibold py-3 lg:py-5 bg-blue-500 hover:bg-blue-400 rounded-t-lg">
                Transactions</p>
        </div>
        <q-card>
            <q-card-section class="flex w-full px-8 py-4 bg-gray-800 mb-5">
                <div class="grid xl:grid-cols-4 lg:grid-cols-2 gap-3 w-full">
                    ...
                    <div class="col-span-1 flex flex-col justify-between">
                        ...
                        <q-btn-dropdown no-caps color="blue-grey-9">
                            ...
                            <q-list>
                                ...
                                <q-item clickable v-close-popup @click="filterCurrency = currency.code"
        ...
        <q-card>
            <q-card-section class="flex w-full p-8 bg-gray-800">
                <p class="font-bold text-lg text-center">Transaction History</p>
                <q-table ref="table" :rows="store.state.transactionHistory" :columns="columns" row-key="name" class="w-full"
                    :loading="loading">
                    <template v-slot:body="props">
                        <q-tr :props="props">
                            ...
                            <q-td key="status" :props="props">
                                <q-badge v-if="props.row.status == 'waiting'" color="gray" class="px-2 py-1">
                                    {{ props.row.status }}
                                </q-badge>
                                <q-badge v-if="props.row.status == 'finished'" color="green" class="px-2 py-1">
                                    {{ props.row.status }}
                                </q-badge>
                                <q-badge v-if="props.row.status == 'partially_paid'" color="yellow" class="px-2 py-1">
                                    {{ props.row.status }}
                                </q-badge>
                                <q-badge v-if="props.row.status == 'failed'" color="red" class="px-2 py-1">
                                    {{ props.row.status }}
                                </q-badge>
                                <q-badge v-if="props.row.status == 'expired'" color="pink" class="px-2 py-1">
                                    {{ props.row.status }}
                                </q-badge>
                            </q-td>
...
onBeforeMount(async () => {
    ...
    await getAvailableCurrencies("transactions", store, router);
